...
{
    static Color color1 = new Color(1f, 0f, 0f); // RED for splat map
	static Color color2 = new Color(0f, 1f, 0f); // GREEN for splat map
	static Color color3 = new Color(0f, 0f, 1f); // BLYE for splat map

    public int index;
    HexCell[] cells;
    public HexMesh terrain, streams, roads, water, waterShore, estuaries;
    Canvas gridCanvas;
    ...
    )
    {
            ...
            if (!cell.IsUnderwater)
            {
                ...
                else if (cell.Elevation > neighbour.WaterLevel)
                {
                    TriangulateWaterfallInWater(
                        e1.v2, e1.v4, e2.v2, e2.v4,
                        cell.StreamSurfaceY, neighbour.StreamSurfaceY,
                        neighbour.WaterSurfaceY
                    );
                }
            }
            else if (
                !neighbour.IsUnderwater &&
                neighbour.Elevation > cell.WaterLevel
            )
            {
                TriangulateWaterfallInWater(
                    e2.v4, e2.v2, e1.v4, e1.v2,
                    neighbour.StreamSurfaceY, cell.StreamSurfaceY,
                    cell.WaterSurfaceY
                );
            }
        }
        ...
        else
        {
			TriangulateEdgeStrip(
				...
				e2, color2, neighbour.TerrainTypeIndex, 
    ...
    )
    {
		...
		if (cell.HasStreamThroughEdge(direction)) {
			TriangulateEstuary(e1, e2, cell.IncomingStream == direction);
		}
		else {
    ...
    )
    {
        ...
        streams.AddQuadUnperturbed(v1, v2, v3, v4);
        streams.AddQuadUV(0f, 1f, 0.8f, 1f);
    }
	void TriangulateEstuary (EdgeVertices e1, EdgeVertices e2, bool incomingStream) {
